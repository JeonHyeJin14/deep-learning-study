...
#result
(tensor([3., 4.]), tensor([1, 1])) #행 방향으로 내려가면서 연산, [1,1]은 인덱스 값
...
print(t.max(dim=1))
print(t.max(dim=-1)) #dim=-1은 마지막 차원제거 : 열 제거
...
torch.Size([2, 2, 3])
```
<img width="326" height="297" alt="image" src="https://github.com/user-attachments/assets/bf841dd4-bb5e-4d82-a335-78c0fe88f336" />
...
```
view([-1,3])의 의미
- -1 : 첫 번째 차원은 사용자가 잘 모르겠으니 파이토치에 맡김
- 3 : 두 번째 차원의 길이는 3을 가지도록 함
<img width="310" height="298" alt="image" src="https://github.com/user-attachments/assets/b7844978-0657-421c-a20d-e92099481f83" />
...
tensor([[0.],
        ...
        [2.]])
torch.Size([3, 1])
# 3*1 크기를 갖는 텐서 -> 스퀴즈를 사용하면 (3,)의 크기를 갖는 텐서로 변경됨
...
tensor([[0., 1., 2.]])
torch.Size([1, 3])
# (3,)를 가졌던 1차원 벡터가 91,3)의 2차원 텐서로 변경된 것을 보여줌
...
tensor([[0., 1., 2.]])
torch.Size([1, 3])
# 언스퀴즈와 view가 동일한 결과를 만듦

#언스퀴즈 인자로 1을 넣으면 두 번째 차원에 1을 추가함
...
# 언스퀴즈 인자로 -1을 넣으면 마지막 차원에 1인 차원을 추가 = 위의 경우랑 같음
print(ft.unsqueeze(-1))
print(ft.unsqueeze(-1).shape)
...
### 7) 타입 캐스팅 (Type Casting) : 자료형 변환
<img width="598" height="254" alt="image" src="https://github.com/user-attachments/assets/fe7080a9-e6cb-4b2d-a7b6-fc0cd0b68472" />

```python
lt = torch.LongTensor([1, 2, 3, 4])
print(lt.float()) #바로 float형으로 타임이 변경됨
...
#두 텐서 연결 : dim=0은 첫 번째 차원을 늘리라는 의미 = 행 기준으로 밑으로 연결
print(torch.cat([x, y], dim=0))

#result -> 두 개의 (2 × 2) 텐서가 (4 × 2) 텐서가 됨
tensor([[1., 2.],
        [3., 4.],
        [5., 6.],
        [7., 8.]])

#dim=0은 첫 번째 차원을 늘리라는 의미 = 열 기준으로 옆으로 연결
print(torch.cat([x, y], dim=1)) 

#result -> 두 개의 (2 × 2) 텐서가 (2 × 4) 텐서가 됨
tensor([[1., 2., 5., 6.],
        [3., 4., 7., 8.]])
```
- 딥 러닝에서는 주로 모델의 입력 또는 주악ㄴ 연산에서 두개의 텐서를 연결하는 경우가 많음
...
<img width="267" height="398" alt="image" src="https://github.com/user-attachments/assets/9ebd1be0-e41f-4f87-b573-4941b03661a4" />

```python
print(torch.cat([x.unsqueeze(0), y.unsqueeze(0), z.unsqueeze(0)], dim=0))

#x, y,z 는 기존에 전부 (2,)의 크기를 가짐 -> 언스퀴즈로 3개의 벡터는 (1,2)의 행렬로 변경됨
#concatenate로 구현할 경우 더 복잡함
#result
tensor([[1., 4.],
        [2., 5.],
        [3., 6.]])

#dim=1인자 : 두 번째 차원 (열)이 증가하도록 쌓으라는 의미
...
tensor([[1., 2., 3.],
        [4., 5., 6.]])
```
- 두번째 차원이 증가하도록 스택킹이 된 결과 -> 결과적으로 (2 × 3) 텐서가 됨
<img width="435" height="204" alt="image" src="https://github.com/user-attachments/assets/1275a32b-6e7d-4c1b-8e3b-c740cc39234f" />
...
# ones_like : 입력 텐서와 크기를 동일하게 하면서 값을 1로 채움
...
#result
# 곱하기 2가 수행됨 -> 기존 x는 변하지 않음
tensor([[2., 4.],
        [6., 8.]])
tensor([[1., 2.],
        [3., 4.]])

#mul뒤에 _를 붙인 연산을 하면 기존 변수에 덮어쓰기 됨
